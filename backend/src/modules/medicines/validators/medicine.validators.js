...
const createMedicine = [
  ...
  body('category').exists({ values: 'falsy' }).isString().trim().isLength({ max: 100 }),
  body('supplier').exists({ values: 'falsy' }).isString().trim().isLength({ max: 120 }),
  ...
  body('batchNumber').exists({ values: 'falsy' }).isString().trim().isLength({ max: 80 }),
  body('batch_number').optional({ checkFalsy: true }).isString().trim().isLength({ max: 80 }),
  body('manufactureDate').exists({ values: 'falsy' }).isISO8601(),
  body('manufacture_date').optional({ checkFalsy: true }).isISO8601(),
