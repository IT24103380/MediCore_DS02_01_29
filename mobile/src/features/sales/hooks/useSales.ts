...
import type { CreateSalePayload, RefundSalePayload, SalesHistoryQuery } from '@/features/sales/types';
...
export function useRefundSale(id?: string) {
  return useMutation({
    mutationFn: (payload: RefundSalePayload) => refundSale(String(id), payload),
    async onSuccess(updatedSale) {
      await Promise.all([
      ...
      queryClient.setQueryData(queryKeys.sales.detail(updatedSale.id), updatedSale);
    },
  });
}
